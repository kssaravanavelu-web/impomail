...
function DraftsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 lg:px-8 lg:py-10">
      ...
      <GmailList items={data ?? []} loading={isLoading} error={error ? (error as Error).message : null} emptyText="No drafts saved." renderActions={(m) => <MailActions id={m.id} actions={["trash"]} />} />
    </div>
  );
}